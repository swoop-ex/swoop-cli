...
const tokenContract = network.loadContract('@swoop-exchange/misc/build/contracts/WONE.json', tokenAddress, 'deployer');
...
async function swapTo() {
  ...
  let result = await tokenInstance.deposit().send({value: amount, ...network.gasOptions()});
...
async function swapFrom() {
  ...
  let result = await tokenInstance.withdraw(amount).send(network.gasOptions());
  let txHash = result.transaction.receipt.transactionHash;
